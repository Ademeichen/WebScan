...
async def test_fixed_tool_workflow():
    ...
    try:
        ...
        executed_nodes = [h["task"] for h in final_state.execution_history]
...
async def test_code_generation_workflow():
    ...
    try:
        ...
        print(f"  目标: {initial_state.target}")
        print(f"  任务ID: {initial_state.task_id}")
        print(f"  需要自定义扫描: {initial_state.target_context.get('need_custom_scan')}")
        ...
        # 验证代码生成节点被执行
        code_gen_found = any(h["task"] == "code_generation" for h in execution_history)
        if code_gen_found:
            print("  ✓ 代码生成节点被执行")
        else:
            print("  ✗ 代码生成节点未执行")
...
async def test_capability_enhancement_workflow():
    ...
    try:
        ...
        print(f"  目标: {initial_state.target}")
        print(f"  任务ID: {initial_state.task_id}")
        print(f"  需要功能增强: {initial_state.target_context.get('need_capability_enhancement')}")
        ...
        # 验证功能补充节点被执行
        enhance_found = any(h["task"] == "capability_enhancement" for h in execution_history)
        if enhance_found:
            print("  ✓ 功能补充节点被执行")
        else:
            print("  ✗ 功能补充节点未执行")
